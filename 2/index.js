...
function listOfStringsToMatrix(listOfStrings) {
  var matrix = [];
  for (var i in listOfStrings) {
    var stringArray = listOfStrings[i].split(/ +/);
    var intArray = stringArray.map(x => parseInt(x));
    matrix.push(intArray);
  }
  return matrix;
}
...
function checksumRow(row) {
  ...
  for (var cellIndex = 1; cellIndex < row.length; cellIndex++) {
    var cell = row[cellIndex];

    if (cell < lowestNumber) {
      lowestNumber = cell;
    } else if (cell > highestNumber) {
      highestNumber = cell;
    }
  }
...
function doChecksum(matrix) {
  var sum = 0;
  for (var rowIndex in matrix) {
    sum += checksumRow(matrix[rowIndex]);
  }
  return sum;
}
